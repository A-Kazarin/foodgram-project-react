...
# Продуктовый помощник Foodgram - мой дипломный проект. Сейчас доступен по адресу: 51.250.26.242
Админка доступна:
...
```bash
docker login -u alexanderkazarin
```
...
```bash
docker build -t alexanderkazarin/foodgram_backend:latest .
```

```bash
docker push alexanderkazarin/foodgram_backend:latest
```
...
```bash
docker build -t alexanderkazarin/foodgram_frontend:latest .
```

```bash
docker push alexanderkazarin/foodgram_frontend:latest
```
...
Автор: [Александр Казарин](https://github.com/A-Kazarin)